...
let changeHelloButton=()=>{
    sayHelloButton.style.backgroundColor= "black";
    sayHelloButton.style.color = "white";
    //let helloBtnColorClass =
}
// PROBLEM 3
/*
...
// CODE HERE
let changeHelloButtonBack=()=>{
    sayHelloButton.style.
...
/*
    ...
    Handle the promise that's returned with a .then, which you should pass a callback function to. Inside the callback function, console.log the response's data (in the intermediate instructions we'll come back to this function and add HTML).
     #8.On each iteration of the loop, create a new p element. Set its textContent equal the string at the current index (i) and then append the new p element onto the document's body.
*/ 
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/tiger').then((res) => {
        console.log(res.data); 
...
let queryFunction = ()=>{
    axios.get('http://localhost:3000/query-test/?name=jesha&favVolor=blue')
    .then(res=>{
        console.log(res.data); 
    })
    .catch(err=>console.log(err));
}
document.getElementById('query-button').addEventListener('click', queryFunction)
  // OR displaying it on the browser  
// const queryFunction = () => {
//     axios.get('http://localhost:3000/query-test?keyexample=valueexample%27).then((res) => {
//         document.getElementById('new-query').textContent = res.data;
//         document.getElementById('new-query').style.display = 'block';
...
const foodInputs =(evt)=>{
    ...
    then((res) => {
        ...
        // or foodList.textContent="";
        for (let i = 0; i < resData.length; i++) {
            let foodItem = document.createElement("li")
            foodItem.append(resData[i]);
            // OR foodItem.textContent =resData[i];
 ...
 createForm.addEventListener("submit",foodInputs)
